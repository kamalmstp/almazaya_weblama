...
class For_management extends Base_Controller {
    public function __construct(){
        parent::__construct();
        $this->idpages = $this->db->query('SELECT pages.title,pages.id,pages.link,language.id as idlang,pages.sister,pages.idmodule,pages.parent,language.lang_code,language.use_default FROM pages INNER JOIN language on pages.idlanguage=language.id WHERE pages.link = "'.$this->link.'" AND language.lang_code = "'.$this->bahasa.'"')->result();
    ...
    public function index(){
        ...
        $this->data['content'] = $this->db->query('SELECT a.* FROM management_list a LEFT JOIN pages b ON a.management_category = b.sister WHERE b.link = "'.$this->parameter.'" AND a.hapus = 0 AND a.draft = 0 AND a.idlanguage='.$this->idpages[0]->idlang)->result_array();
        /*Cek IF HAS SUBMENU Will Direct to first submenu */
        
        //$this->cekHasSubmenu();
        //echo "<pre>";
        //print_r('SELECT pages.title,pages.id,pages.link,language.id as idlang,pages.sister,pages.idmodule,pages.parent,language.lang_code,language.use_default FROM pages INNER JOIN language on pages.idlanguage=language.id WHERE pages.link = "'.$this->link.'" AND language.lang_code = "'.$this->bahasa.'"');
        ...
        $this->data['category'] = $this->db->query('SELECT a.* FROM `management_category` a LEFT JOIN language b ON a.idlanguage=b.id WHERE b.lang_code="'.$this->data['lang_code'].'" ')->result_array();
        $this->data['jenis_pertanyaan'] = $this->db->query('SELECT * from pengaduan_nasabah_jenis_pertanyaan WHERE idlanguage='.$this->idpages[0]->idlang)->result_array();
        $this->load->view('management',$this->data);
    }
    function cekHasSubmenu(){
        
        $idpages = $this->db->query('SELECT pages.title,pages.id,pages.link,language.id as idlang,pages.sister,pages.idmodule,pages.parent,language.lang_code,language.use_default FROM pages INNER JOIN language on pages.idlanguage=language.id WHERE pages.parent = "'.$this->idpages[0]->sister.'" AND language.lang_code = "'.$this->bahasa.'"')->result();
        
        if (count($idpages) > 1){
            redirect($this->data['codebahasa'].$idpages[0]->link, 'refresh');
        }
    }
    ...
    function sidemenu($parent){
        ...
        $sidemenunya = $this->db->query('SELECT pages.parent,pages.title,pages.link,pages.id,language.id as idlang,pages.sister,pages.idmodule,language.lang_code,language.use_default,(SELECT count(*) FROM pages a INNER JOIN language on a.idlanguage=language.id WHERE a.parent = pages.sister AND language.lang_code = "'.$this->bahasa.'") AS total FROM pages INNER JOIN language on pages.idlanguage=language.id WHERE pages.parent = "'.$sidemenunya[0]->parent.'" AND language.lang_code = "'.$this->bahasa.'" ORDER BY pages.posisi ASC')->result();
         //echo "<pre>";
        //print_r($sidemenunya);
    ...
    function getReportOtherLang($idlang,$link){
        $pagenya = $this->db->query('SELECT a.title,link FROM pages a WHERE a.sister = (SELECT sister FROM `pages` WHERE `link` LIKE "'.$link.'") AND idlanguage = '.$idlang.'')->result_array();
