...
class Consumer_banking_group_page extends Base_Controller {
    public function __construct(){
        parent::__construct();
        $this->idpages = $this->db->query('SELECT pages.link,pages.title,pages.id,language.id as idlang,pages.sister,pages.idmodule,pages.parent,language.lang_code,language.use_default FROM pages INNER JOIN language on pages.idlanguage=language.id WHERE pages.link = "'.$this->link.'" AND language.lang_code = "'.$this->bahasa.'"')->result();
    ...
    public function index(){
        ...
        $this->data['linkpage'] = $this->link_bahasa.$this->idpages[0]->link."/";
        
        //echo "<pre>";
        //print_r($this->getChildMenu($this->idpages[0]->sister,$this->idpages[0]->idlang));
        
        $this->data['rec'] = $this->getChildMenu($this->idpages[0]->sister,$this->idpages[0]->idlang);

        // echo "<pre>";
        // print_r($this->data['rec']);
        // die();

        $this->load->view('consumer_banking_group',$this->data);
    }
    ...
    function getChildMenu($parent,$idlang){
        $sidemenunya = $this->db->query('SELECT pages.title,pages.link,pages.id,pages.sister,pages.idmodule,(SELECT pic FROM produklayananconsumerbanking_category WHERE produklayananconsumerbanking_category.idpages=pages.sister AND produklayananconsumerbanking_category.idlanguage='.$idlang.') as picnya FROM pages WHERE pages.parent = "'.$parent.'" AND pages.idlanguage='.$idlang.' AND pages.draft=0 AND pages.hapus=0 ORDER BY pages.posisi ASC')->result();
        return $sidemenunya;
    }
