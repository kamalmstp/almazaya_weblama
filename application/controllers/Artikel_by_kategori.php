...
class Artikel_by_kategori extends Base_Controller {
    ...
    public function index(){
        ...
        if ($this->parameter != ""){
            ...
            $query_detail = 'SELECT a.*,DATE_FORMAT(a.cretime,"%d %M %Y") AS datenya,b.link as link_pages FROM `artikel` a  LEFT JOIN pages b ON b.id=a.cat WHERE a.cat='.$this->idpages[0]->id.' AND  a.idlanguage = '.$this->idpages[0]->idlang.' AND a.link LIKE "'.$this->parameter.'"';
            ...
            $query_related = 'SELECT a.*,DATE_FORMAT(a.cretime,"%d %M %Y") AS datenya,b.link as link_pages FROM `artikel` a  LEFT JOIN pages b ON b.id=a.cat WHERE a.cat='.$artikel_detail[0]["cat"].'  AND a.link != "'.$this->parameter.'" AND  a.idlanguage = '.$this->idpages[0]->idlang.' LIMIT 0,3';
...
//            print_r($data_rec_ads_banner_global);
            ...
            if ($record > 0){
                ...
                $query_others = 'SELECT a.*,DATE_FORMAT(a.cretime,"%d %M %Y") AS datenya,b.link as link_pages,(select pages.title FROM pages where record = a.idmenu) as cat FROM aktifitas a LEFT JOIN pages b ON b.id='.$this->idpages[0]->id.' WHERE a.idmenu='.$this->idpages[0]->record.' AND a.hapus=0  order by a.cretime  DESC LIMIT  '.$start.','.$limit;

                $query_others_count = 'SELECT * FROM aktifitas WHERE idmenu='.$this->idpages[0]->record.' AND hapus=0  order by cretime DESC ';
                ...
                $query_others = 'SELECT a.*,DATE_FORMAT(a.cretime,"%d %M %Y") AS datenya,b.link as link_pages,(select pages.title FROM pages where record = a.idmenu) as cat,(select pages.link FROM pages where record = a.idmenu) as link_pages FROM aktifitas a LEFT JOIN pages b ON b.id='.$this->idpages[0]->id.' WHERE a.hapus=0 order by a.cretime  DESC LIMIT  '.$start.','.$limit;

                //echo $query_others;

                $query_others_count = 'SELECT * FROM aktifitas WHERE hapus=0  order by cretime DESC ';
                ...
                $query_others = 'SELECT a.*,DATE_FORMAT(a.cretime,"%d %M %Y") AS datenya,b.link as link_pages,b.title as cat FROM artikel a LEFT JOIN pages b ON a.cat=b.id WHERE a.hapus=0 AND a.deleted=0 AND a.cat='.$this->idpages[0]->id.'  order by a.cretime  DESC LIMIT  '.$start.','.$limit;

                $query_others_count = 'SELECT a.*,DATE_FORMAT(a.cretime,"%d %M %Y") AS datenya,b.link as link_pages,b.title as cat FROM artikel a LEFT JOIN pages b ON a.cat=b.id WHERE a.hapus=0 AND a.deleted=0 AND a.cat='.$this->idpages[0]->id.'  order by a.cretime  DESC';
                ...
                //echo 'SELECT record,meta_title,meta_description,pages.link,pages.pic,pages.content,pages.title,pages.id,language.id as idlang,pages.sister,pages.idmodule,pages.parent,language.lang_code,language.use_default FROM pages INNER JOIN language on pages.idlanguage=language.id WHERE pages.parent = "'.$this->idpages[0]->id.'" AND language.lang_code = "'.$this->bahasa.'"';

                //$this->toppages = $this->db->query('SELECT meta_title,meta_description,pages.link,pages.pic,pages.content,pages.title,pages.id,language.id as idlang,pages.sister,pages.idmodule,pages.parent,language.lang_code,language.use_default FROM pages INNER JOIN language on pages.idlanguage=language.id WHERE pages.id = "'.$this->idpages[0]->parent.'" AND language.lang_code = "'.$this->bahasa.'"')->result();

                

                

                //print_r($this->idpages_sub);
            ...
            $this->load->view('aktifitas',$this->data);
