...
class Activation extends Base_Controller {
    public function __construct(){
        ...
        //echo $this->idpages[0]->lang_code;
        //$this->output->nocache();
        //echo $this->data['codebahasa'];
    ...
    public function index(){
        ...
        //echo $this->encrypt->decodeUrl($this->segs[2],$this->key);
        if (isset($this->segs[2])){
            $data = array(
               'active' => 1
            );
            $this->db->where('uid', $this->encrypt->decodeUrl($this->xss_filter($this->segs[2]),$this->key));
            $this->db->update('account', $data);
        }
        
        $menu_login = $this->db->query('SELECT link FROM pages WHERE idmodule = 49 AND idlanguage = '.$this->idpages[0]->idlang)->result_array();
        if (count($menu_login) > 0){
            $this->data["menu_login"] = $menu_login[0]["link"];  
        }else{
            $this->data["menu_login"] = "";  
        }
