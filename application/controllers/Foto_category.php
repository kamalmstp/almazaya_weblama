...
class Photo extends Base_Controller {
    ...
    public function index(){
        ...
        if ($this->parameter != ""){
            ...
            $count_artikel = $this->db->query('SELECT count(*) as total FROM `video` a  LEFT JOIN kategori_foto b ON a.cat=b.id LEFT JOIN pages c ON b.idpages=c.id where a.deleted=0 AND a.hidden=0 AND a.idlanguage = '.$this->idpages[0]->idlang.' AND b.link LIKE "'.$this->parameter.'" order by a.posisi')->result_array();
        ...
        $this->data['menu'] = $this->db->query('SELECT a.id,a.title,a.link,a.parent,language.lang_code,a.submenu,a.sister,(SELECT COUNT(*) FROM pages d WHERE a.sister=d.parent AND d.idlanguage=language.id) AS hasubmenu FROM pages a INNER JOIN language on a.idlanguage = language.id  WHERE a.parent = 0 AND a.hapus = 0 AND a.draft = 0 AND language.lang_code = "'.$this->bahasa.'" ORDER BY a.posisi ASC')->result_array();
        ...
        $newartikel = $this->db->query('SELECT a.title,a.thumb,a.pic,a.cretime,a.link as link_news,c.title as title_cat,d.link as link_detail FROM `artikel` a LEFT JOIN kategori_artikel c ON a.cat=c.id LEFT JOIN pages d ON c.idpages=d.id where a.deleted=0 AND a.hidden=0 AND a.idlanguage = '.$this->idpages[0]->idlang.' AND a.title != "" order by a.posisi DESC limit 0,5')->result_array();
        ...
        $this->load->view('listcatvideo',$this->data);
    }
