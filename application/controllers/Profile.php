...
class Profile extends Base_Controller {
    ...
    public function index(){
        ...
        $query = $this->db->query('SELECT *,DATE_FORMAT(crdate,"%d %M %Y") AS datenya,DATE_FORMAT(birthdate,"%d %M %Y") AS birthdatenya FROM account WHERE uid = '.$data_session["login_user"]["eternaleaf_user_login"])->result_array();;
        $query_alamat = $this->db->query('SELECT a.*,b.province,c.city_name,d.subdistrict_name FROM address a LEFT JOIN province b ON a.uid_province=b.province_id LEFT JOIN city c ON a.uid_city=c.city_id LEFT JOIN subdistrict d ON a.uid_subdistrict=d.subdistrict_id WHERE a.uid_account = '.$query[0]["uid"].' AND a.default = 1')->result_array();;
