...
//        print_r($data_session);
        ...
        if (isset($data_session["jne"])){
            $this->data["shippingfee"] = $data_session["jne"]["ongkir"];
        }
        
        
        //echo 'SELECT pages.parent,pages.link,pages.pic,pages.content,pages.title,pages.id,language.id as idlang,pages.sister,pages.idmodule,pages.parent,language.lang_code,language.use_default FROM pages INNER JOIN language on pages.idlanguage=language.id WHERE pages.parent = "'.$this->idpages[0]->parent.'" AND language.lang_code = "'.$this->bahasa.'" AND pages.posisi > '.$this->idpages[0]->posisi. ' LIMIT 0,1';
        ...
        //        print_r($top);
                ...
                $this->data["totaltobepaid"] = (int)$top[$no]["totalhargaall"] + (int)$data_session["jne"]["ongkir"];
                
                $this->session->userdata['jne']['ongkir'] = (int)$data_session["jne"]["ongkir"];
                $this->session->userdata['jne']['etd'] = $data_session["jne"]["etd"];
                $this->session->userdata['total_harga']["harga_order"] = $top[$no]["totalhargaall"];
                $this->session->userdata['total_harga']["harga_dibayar"] = (int)$top[$no]["totalhargaall"] + (int)$data_session["jne"]["ongkir"];
                
            }
            
        }
        $query_bank = $this->db->query('SELECT a.* FROM bank_account a WHERE draft=0 AND hapus=0 AND idlanguage = '.$this->idpages[0]->idlang)->result_array();;
        $this->data["bank"] = $query_bank;
