...
class Be_about extends Webadmin_Controller {
  public function __construct(){
    ...
    public function index(){ 
        ...
        $this->content($query[0]->id,$query[0]->parent,$query[0]->title,$query[0]->width,$query[0]->height,$query[0]->module);
        //redirect('/'.$this->admin.'/'.$link, 'refresh');
    }
  function content($id,$parent,$title,$width,$height,$module){
        ...
        if (isset($form) AND $form == 1 ){
            if (count($data_rec) > 0){
                $status = $this->_edit_kategoriartikel($id,$parent,$width,$height,$module);
            }else{
                $status = $this->_save_kategoriartikel($id,$parent,$width,$height,$module);
            }
            $data_rec = $this->db->query('SELECT * FROM `content` WHERE idmenu = '.$id)->result_array(); 
            $data['rec'] = $data_rec;
            $data['status'] = $status;
    ...
    function _edit_kategoriartikel($id,$parent,$widthnya,$heightnya,$module){
        //print_r($_POST);
        //die();
    ...
    function _save_kategoriartikel($id,$parent,$widthnya,$heightnya,$module){
        //print_r($_POST);
        //die();
        $idlang   = $this->input->post('idlang');
        $title    = $this->input->post('title');
