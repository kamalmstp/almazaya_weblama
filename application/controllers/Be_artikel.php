...
class Be_artikel extends Webadmin_Controller {
        ...
        if (isset($this->segs[3]) AND $this->segs[3] == 'act'){           
          if (isset($this->segs[4])){
            ...
            if ($this->segs[4] == 'add'){
              ...
              $data['cat'] = $this->db->query('SELECT * FROM `pages` WHERE idlanguage = '.$deflang.' AND idmodule=85 AND draft = 0 AND hapus = 0 ORDER BY posisi asc')->result_array(); 
            ...
            $data_rec = $this->db->query('SELECT artikel.id,artikel.title,artikel.hidden,artikel.deleted,artikel.sister,artikel.thumb,artikel.pic, pages.title as pages FROM `artikel` LEFT JOIN pages ON artikel.cat=pages.sister   where artikel.idlanguage = '.$deflang.' '.$sqlwhere.' AND artikel.idmenu='.$id.' order by artikel.posisi desc LIMIT '.$start.','.$limit)->result_array();  
            $data_rec_count = $this->db->query('SELECT artikel.id,artikel.title,artikel.hidden,artikel.deleted,artikel.sister,artikel.thumb,artikel.pic, pages.title as pages FROM `artikel` LEFT JOIN pages ON artikel.cat=pages.sister   where artikel.idlanguage = '.$deflang.' '.$sqlwhere.' AND artikel.idmenu='.$id.' order by artikel.posisi desc')->result_array();
        ...
        foreach ($idlang as $lang) {
            ...
            if ($step1 == TRUE AND isset($_FILES['pic'.$lang]['name']) AND $_FILES['pic'.$lang]['name'] != '') {
                ...
                if (move_uploaded_file($_FILES['pic'.$lang]['tmp_name'], FCPATH.'/uploads/artikel/'.$new_file_name)) {//if file moved to uploads folder
                    $this->resize_crop_image(400, 300, FCPATH.'/uploads/artikel/'.$new_file_name, FCPATH.'/uploads/artikel/thumb_'.$new_file_name);
                    }
            }
