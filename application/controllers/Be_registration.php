...
class Be_registration extends Webadmin_Controller {
  ...
  public function banner($id,$parent,$title,$link){
    ...
    if (isset($this->segs[3]) AND $this->segs[3] == 'act'){           
      ...
      $data_rec = $this->db->query('SELECT * FROM `registration` where 1=1 '.$sqlwhere.' order by  uid desc')->result_array();  
      $data_count = $this->db->query('SELECT count(*) as jumlah FROM `registration` where 1=1 '.$sqlwhere.' order by uid desc')->result();        
      $data['rec']   = $data_rec;
      $data['act'] = 'default';
      //echo "<pre>";
      //print_r($data_rec);
