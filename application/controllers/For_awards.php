...
class For_awards extends Base_Controller {
    ...
    public function index(){
        ...
        $data_rec = $this->db->query('SELECT penghargaan.thumb,penghargaan.id,penghargaan.title,penghargaan.sister,penghargaan.subtitle, pages.title as pages FROM `penghargaan` INNER JOIN pages ON penghargaan.idpages=pages.id where penghargaan.idlanguage = '.$this->idpages[0]->idlang.' AND penghargaan.hapus=0 AND penghargaan.draft=0 order by penghargaan.cretime DESC')->result_array(); 
        ...
        $gallery = $this->db->query('SELECT penghargaan.thumb,penghargaan.id,penghargaan.title,penghargaan.sister,penghargaan.subtitle, pages.title as pages FROM `penghargaan` INNER JOIN pages ON penghargaan.idpages=pages.id where penghargaan.idlanguage = '.$this->idpages[0]->idlang.' AND penghargaan.hapus=0 AND penghargaan.draft=0 order by penghargaan.cretime DESC');
        $this->data['title'] = 'penghargaan';
        $this->load->library('pagination');
        $config['base_url'] = base_url().$this->data['codebahasa'].$this->link.'/';
        $config['total_rows'] = $gallery->num_rows();
        ...
        //$uri = ($this->uri->segment(3)!=null) ? $this->uri->segment(3) : 1;        
        $config['cur_page'] = $uri;
        /*if ($uri != 0){
            $uri = $uri - 1;
        }*/
        ...
        $config['prev_link'] = '<div class="prev">&lt;</div>';
        $config['next_link'] = '<div class="next">&gt;</div>';        
        $config['page_query_string'] = TRUE;
        $config['query_string_segment'] = 'page';
        $this->pagination->initialize($config);
        $gallery_limit = $this->db->query('SELECT penghargaan.thumb,penghargaan.pic,penghargaan.id,penghargaan.title,penghargaan.sister,penghargaan.subtitle, pages.title as pages FROM `penghargaan` INNER JOIN pages ON penghargaan.idpages=pages.id where penghargaan.idlanguage = '.$this->idpages[0]->idlang.' AND penghargaan.hapus=0 AND penghargaan.draft=0 order by penghargaan.cretime  DESC limit ' .$uri.",".$limit)->result_array();
        //echo "select a.*,(SELECT count(fbid) FROM vote WHERE uid_gallery=a.uid) as vote from gallery a ORDER BY vote DESC limit ".$uri.",".$limit;
        $this->data["gallery"] = $gallery_limit;
        $this->data["page"] = $this->pagination->create_links(); 
        //echo "<pre>";
        //print_r($gallery_limit);
        $data["active"] = "all";
        
        $this->load->view('awards',$this->data);
    }
