    ...
    public function index(){
        ...
        if (isset($_POST["email"])){
            ...
            if (count($query) > 0){
                ...
                $data_akun = $this->pages_account();
                //echo "<pre>";
                //print_r($data_akun);
                redirect(base_url($this->data['codebahasa'].$data_akun[0]["link"]), 'refresh');
            }else{
                redirect(base_url($this->data['codebahasa'].$this->idpages[0]->link."/?error=".$this->lang->line('error_login')), 'refresh');
            }
            
            
        }
        
        $menu_forgot_password = $this->db->query('SELECT link FROM pages WHERE idmodule = 64 AND idlanguage = '.$this->idpages[0]->idlang)->result_array();
        if (count($menu_forgot_password) > 0){
            $this->data["menu_forgot"] = $menu_forgot_password[0]["link"];  
        }else{
            $this->data["menu_forgot"] = "";  
        }
        $menu_registration = $this->db->query('SELECT link FROM pages WHERE idmodule = 50 AND idlanguage = '.$this->idpages[0]->idlang)->result_array();
        if (count($menu_registration) > 0){
            $this->data["menu_registration"] = $menu_registration[0]["link"];  
        }else{
            $this->data["menu_registration"] = "";  
        }
