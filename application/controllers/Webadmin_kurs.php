...
class Webadmin_kurs extends Webadmin_Controller {
    ...
    function _save_kurs(){
        ...
        $title   = $this->input->post('mata_uang');
        $indicative_special_rate_jual   = $this->input->post('indicative_special_rate_jual');
        $indicative_special_rate_beli   = $this->input->post('indicative_special_rate_beli');
        $ttcounter_jual = $this->input->post('ttcounter_jual');
        $ttcounter_beli = $this->input->post('ttcounter_beli');
        $bank_notes_jual = $this->input->post('bank_notes_jual');
        $bank_notes_beli = $this->input->post('bank_notes_beli');
        ...
        if($step1 == TRUE AND $success == TRUE){
            ...
            $inputdata = array( 
                'title'      => $title, 
                'indicative_special_rate_jual' => $indicative_special_rate_jual,
                'indicative_special_rate_beli' => $indicative_special_rate_beli,
            ...
            $saved = $this->db->insert('kurs', $inputdata);
        }
    ...
    function _edit_kurs(){
        ...
        if($step1 == TRUE AND $success == TRUE){
            ...
            $this->db->where('id', $id);
            $edit = $this->db->update('kurs', $editdata); 
        }
    ...
    public function kurs(){
        ...
        if (isset($this->segs[3]) AND $this->segs[3] == 'act'){   
          ...
          if (isset($this->segs[4])){
            ...
            if ($this->segs[4] == 'add'){
              ...
              if (isset($this->segs[5]) AND $this->segs[5] != '' AND is_numeric($this->segs[5])) {
                $form = $this->input->post('form');
                if (isset($form) AND $form == 1 ){
                  $status = $this->_edit_kurs();
                  $data['status'] = $status;
                }
                $data_rec = $this->db->query('SELECT * FROM `kurs` WHERE sister = '.$this->segs[5])->result_array(); 
                if ($data_rec) {
                  $data['rec'] = $data_rec;
                } else {
              ...
              if (isset($this->segs[5]) AND $this->segs[5] != '' AND is_numeric($this->segs[5])) {
                $data_row = $this->db->query('SELECT id,title,link,pic,thumb,datestart,dateend,draft FROM `kurs` WHERE id = '.$this->segs[5])->result_array(); 
                if ($data_row) {
                  $data['row'] = $data_row[0];
                } else {
                  redirect('/webadmin/'.$this->url_module, 'refresh');
              ...
              if (isset($this->segs[5]) AND $this->segs[5] != '') {
                $deleted = $this->deleteData('kurs',$this->segs[5]);
                if ($deleted){
              ...
              $this->_ispermit($permit->isdelete);
              if (isset($this->segs[5]) AND $this->segs[5] != '') {
                $restore = $this->restoreData('kurs',$this->segs[5]);
                if ($restore){
                  redirect($this->agent->referrer(), 'refresh');
                }
              } else {
              ...
              $this->_ispermit($permit->isdelete);
              $trash = $this->trashData_haveParent('kurs',$this->segs[5]);
              if ($trash){
                redirect($this->agent->referrer(), 'refresh');
              }
            }elseif ($this->segs[4] == 'movedraft'){
              $this->_ispermit($permit->isupdate);
              $draft = $this->draftData_haveParent('kurs',$this->segs[5]);
              if ($draft){
                redirect($this->agent->referrer(), 'refresh');
              }
            }elseif ($this->segs[4] == 'movepublish'){
              $this->_ispermit($permit->isupdate);
              $publish = $this->publishData('kurs',$this->segs[5]);
              if ($publish){
                redirect($this->agent->referrer(), 'refresh');
              }
            }else{
          ...
          if (isset($this->segs[3]) AND $this->segs[3] == 'trash') {
            $sqlwhere = 'kurs.hapus = 1';
            $data['statuspage'] = 'trash';
          }elseif (isset($this->segs[3]) AND $this->segs[3] == 'draft') {
            echo "test";
            $sqlwhere = 'kurs.hapus = 0 and kurs.draft = 1';
            $data['statuspage'] = 'draft';
          }else {
            $sqlwhere = "kurs.hapus = 0 and kurs.draft = 0 ";
          ...
          $data_rec = $this->db->query('SELECT * FROM `kurs` where '.$sqlwhere.' order by kurs.posisi asc')->result_array();  
          $data_count = $this->db->query('SELECT count(*) as jumlah FROM `kurs` where '.$sqlwhere.' order by posisi asc')->result();
           
           
          $count_draft = $this->db->query('SELECT count(*) as jumlah FROM `kurs` where kurs.hapus = 0 and kurs.draft = 1 order by posisi asc')->result();
          $count_trash = $this->db->query('SELECT count(*) as jumlah FROM `kurs` where kurs.hapus = 1  order by posisi asc')->result();
          $count_publish = $this->db->query('SELECT count(*) as jumlah FROM `kurs` where kurs.hapus = 0 and kurs.draft = 0 order by posisi asc')->result();
          
           
          $data['rec']        = $data_rec;
          $data['act']        = 'default';
          $data['jumlahdata'] = $data_count[0]->jumlah;
          $data['count_publish']    = $count_publish[0]->jumlah;
          $data['count_draft']      = $count_draft[0]->jumlah;
          $data['count_trash']      = $count_trash[0]->jumlah;
        }
