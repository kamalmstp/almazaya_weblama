...
class Be_list_level extends Webadmin_Controller {
          ...
          if (isset($this->segs[4])){
            ...
            if ($this->segs[4] == 'add'){
              ...
              if (isset($this->segs[5]) AND $this->segs[5] != '' AND is_numeric($this->segs[5])) {
                ...
                $data_rec = $this->db->query('SELECT * FROM `level` WHERE sister = '.$this->segs[5])->result_array(); 
              ...
              if (isset($this->segs[5]) AND $this->segs[5] != '' AND is_numeric($this->segs[5])) {
                $data_row = $this->db->query('SELECT * FROM `level` WHERE id = '.$this->segs[5])->result_array(); 
                if ($data_row) {
                  $data['row'] = $data_row[0];
                } else {
                  redirect('/webadmin/'.$url_module, 'refresh');
              ...
              $deleted = $this->deleted('level',$this->segs[5]);
              if ($deleted){
              ...
              $hide = $this->hide('level',$this->segs[5]);
              ...
              $this->_ispermit($permit->ishide);
              $unhide = $this->unhide('level',$this->segs[5]);
              if ($unhide){
                redirect($this->agent->referrer(), 'refresh');
              }
            }else{
...
//            die();
            $data_rec = $this->db->query('SELECT * FROM `level`  where deleted=0 AND hapus=0 AND idlanguage = '.$deflang.' AND idmenu='.$id.' order by posisi desc LIMIT '.$start.','.$limit)->result_array();  
            $data_rec_count = $this->db->query('SELECT * FROM `level`  where deleted=0 AND hapus=0 AND idlanguage = '.$deflang.' AND idmenu='.$id.' order by posisi desc')->result_array();
            
            if (isset($_GET["cari"])){
                $data_rec = $this->db->query('SELECT * FROM `level`  where deleted=0 AND hapus=0 AND idlanguage = '.$deflang.' AND idmenu='.$id.' AND title LIKE "%'.$_GET["keyword"].'%" order by posisi desc LIMIT '.$start.','.$limit)->result_array();  
                $data_rec_count = $this->db->query('SELECT * FROM `level`  where deleted=0 AND hapus=0 AND idlanguage = '.$deflang.' AND idmenu='.$id.' AND title LIKE "%'.$_GET["keyword"].'%" order by posisi desc')->result_array();
          ...
          $data['count_publish']    = $this->countPublishNewBase('level',$deflang,$id);
          $data['count_draft']      = $this->countDraftNewBase('level',$deflang,$id);
          $data['count_trash']      = $this->countTrashNewBase('level',$deflang,$id);
        }
        if ($parent > 0){
            $data['submenu'] = TRUE;
        }else{
  ...
  function _save_karir($id,$parent){
        ...
        $sister = $this->_nextAI('level');
        $posisi = $this->_nextPosisi('level',$id);
        $posisi = $posisi + 1;
        $notice = "";
        $step1   = TRUE;
        $success = TRUE;
        ...
        foreach ($idlang as $lang) {
            ...
            if ($step1 == TRUE AND isset($_FILES['pic'.$lang]['name']) AND $_FILES['pic'.$lang]['name'] != '') {  
                ...
                move_uploaded_file($_FILES['pic'.$lang]['tmp_name'], FCPATH.'uploads/level/'.$new_file_name);
            }
            $new_icon = "";
            if ($step1 == TRUE AND isset($_FILES['icon'.$lang]['name']) AND $_FILES['icon'.$lang]['name'] != '') {  
                ...
                move_uploaded_file($_FILES['icon'.$lang]['tmp_name'], FCPATH.'uploads/level/'.$new_icon);
            }
          if($step1 == TRUE AND $success == TRUE){
            ...
            $saved = $this->db->insert('level', $inputdata);
          }
          $filename = "";
    ...
    function _edit_karir(){
        ...
        foreach ($idlang as $lang) {
            ...
            $edit = $this->db->update('level', $editdata); 
          }
        }
        if($step1 == TRUE AND $success == TRUE){
          if ($edit){
