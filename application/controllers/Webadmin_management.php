...
class Webadmin_management extends Webadmin_Controller {
    ...
    function _save_management_category(){
        ...
        $sister = $this->_nextAI('management_category');
        $notice  = "";
        $success = TRUE;
        $step1   = TRUE;
...
//        $thumbname = "";
        ...
        foreach ($idlang as $lang) {
            ...
            if($step1 == TRUE AND $success == TRUE){
                ...
                $saved = $this->db->insert('management_category', $inputdata);
            }
        }
        if($step1 == TRUE AND $success == TRUE){
            if ($saved){
    ...
    function _edit_management_category(){
        // echo '<pre>';
        // print_r($_POST);
        // print_r($_FILES);
        // die();
        ...
        foreach ($idlang as $lang) {
            ...
            $edit = $this->db->update('management_category', $editdata); 
            }
        }
        if($step1 == TRUE AND $success == TRUE){
            if ($edit){
    ...
    public function management(){
        ...
        $this->url_module = "managementcategory";
        $login = $this->_isLogin();
        $idmenudb = 38;
        ...
        if (isset($this->segs[3]) AND $this->segs[3] == 'act'){   
          ...
          if (isset($this->segs[4])){
            ...
            if ($this->segs[4] == 'add'){
              ...
              if (isset($form) AND $form == 1 ){
                ...
                $status = $this->_save_management_category();
                $data['status'] = $status;
              } 
              ...
              if (isset($this->segs[5]) AND $this->segs[5] != '' AND is_numeric($this->segs[5])) {
                $form = $this->input->post('form');
                if (isset($form) AND $form == 1 ){
                  $status = $this->_edit_management_category();
                  $data['status'] = $status;
                }
                $data_rec = $this->db->query('SELECT id,idpages,title,description,link_ori,link FROM `management_category` WHERE sister = '.$this->segs[5])->result_array(); 
                if ($data_rec) {
                  $data['rec'] = $data_rec;
                } else {
              ...
              if (isset($this->segs[5]) AND $this->segs[5] != '') {
                $deleted = $this->deleteData('management_category',$this->segs[5]);
                if ($deleted){
              ...
              $this->_ispermit($permit->isdelete);
              if (isset($this->segs[5]) AND $this->segs[5] != '') {
                $restore = $this->restoreData('management_category',$this->segs[5]);
                if ($restore){
                  redirect($this->agent->referrer(), 'refresh');
                }
              } else {
              ...
              $this->_ispermit($permit->isdelete);
              $trash = $this->trashData_haveParent('management_category',$this->segs[5]);
              if ($trash){
                redirect($this->agent->referrer(), 'refresh');
              }
            }elseif ($this->segs[4] == 'movedraft'){
              $this->_ispermit($permit->isupdate);
              $draft = $this->draftData_haveParent('management_category',$this->segs[5]);
              if ($draft){
                redirect($this->agent->referrer(), 'refresh');
              }
            }elseif ($this->segs[4] == 'movepublish'){
              $this->_ispermit($permit->isupdate);
              $publish = $this->publishData('management_category',$this->segs[5]);
              if ($publish){
                redirect($this->agent->referrer(), 'refresh');
              }
            }else{
          ...
          if (isset($this->segs[3]) AND $this->segs[3] == 'trash') {
            $sqlwhere = 'management_category.hapus = 1';
            $data['statuspage'] = 'trash';
          }elseif (isset($this->segs[3]) AND $this->segs[3] == 'draft') {
            $sqlwhere = 'management_category.hapus = 0 and management_category.draft = 1';
            $data['statuspage'] = 'draft';
          }else {
            $sqlwhere = "management_category.hapus = 0 and management_category.draft = 0 ";
          ...
          $data_rec = $this->db->query('SELECT management_category.id,management_category.title,management_category.sister FROM `management_category` where management_category.idlanguage = '.$deflang.' AND '.$sqlwhere.' AND management_category.parent=0 order by management_category.posisi asc')->result_array();  
          $data_count = $this->db->query('SELECT count(*) as jumlah FROM `management_category` where idlanguage = '.$deflang.' AND '.$sqlwhere.' AND management_category.parent=0 order by posisi asc')->result();
           
          $data['rec']        = $data_rec;
          $data['act']        = 'default';
          $data['jumlahdata'] = $data_count[0]->jumlah;
          $data['count_publish']    = $this->countPublishNew('management_category',$deflang);
          $data['count_draft']      = $this->countDraftNew('management_category',$deflang);
          $data['count_trash']      = $this->countTrashNew('management_category',$deflang);
        }
        $data['url_module'] = $this->url_module;
        $data['submenu'] = FALSE;
        $data['active']  = 'Management';
        $data['active2'] = '';
        $data['page']    = 'managementcategory';
