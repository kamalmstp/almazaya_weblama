...
class Fe_artikel extends Base_Controller {
...
//            print_r($data_rec_ads_banner_global);
            ...
            if ($record > 0){
                ...
                $query_others = 'SELECT a.*,DATE_FORMAT(a.cretime,"%d %M %Y") AS datenya,b.link as link_pages,b.title as cat FROM artikel a LEFT JOIN pages b ON a.cat=b.id WHERE a.hapus=0 AND a.deleted=0 AND a.hidden=0  order by a.cretime  DESC LIMIT  '.$start.','.$limit;
                $query_others_count = 'SELECT a.*,DATE_FORMAT(a.cretime,"%d %M %Y") AS datenya,b.link as link_pages,b.title as cat FROM artikel a LEFT JOIN pages b ON a.cat=b.id WHERE a.hapus=0 AND a.deleted=0 AND a.hidden=0  order by a.cretime  DESC';
                $this->idpages_sub = $this->db->query('SELECT record,meta_title,meta_description,pages.link,pages.pic,pages.content,pages.title,pages.id,language.id as idlang,pages.sister,pages.idmodule,pages.parent,language.lang_code,language.use_default FROM pages INNER JOIN language on pages.idlanguage=language.id WHERE pages.parent = "'.$this->idpages[0]->id.'"')->result();
