...
class Be_artikel_by_kategori extends Webadmin_Controller {
  public function __construct(){
    ...
    public function index(){      
        ...
        $this->cat_skinpedia($query[0]->id,$query[0]->parent,$query[0]->title,3,$query[0]->link);
        //redirect('/'.$this->admin.'/'.$link, 'refresh');
    }
  public function cat_skinpedia($id, $parenttop, $title ,$type,$link){
    ...
    if (isset($this->segs[3]) AND $this->segs[3] == 'act'){          
      if (isset($this->segs[4])){
        ...
        if ($this->segs[4] == 'add'){
          ...
          $data['menuparent'] = $this->db->query('SELECT id,title,sister FROM `pages` WHERE  idmodule=83 AND idlanguage = '.$deflang.' AND hapus=0 AND draft=0 ORDER BY `posisi` asc')->result_array();
      ...
      $data_rec = $this->db->query('SELECT pages.hide_in_menu,pages.type,pages.id,pages.title,pages.sister,module.title as module FROM `pages`  INNER JOIN module ON pages.idmodule = module.id where pages.idlanguage = '.$deflang.' AND '.$sqlwhere.' AND pages.idmodule=85 order by pages.posisi asc')->result_array();
      //print_r($data_rec);
      ...
      //die("test");
      $data_count = $this->db->query('SELECT count(*) as jumlah FROM `pages` where idlanguage = '.$deflang.' AND '.$sqlwhere.'  AND pages.idmodule=85 order by posisi asc')->result();
