...
class Fe_list_galeri extends Base_Controller {
...
//            print_r($data_rec_ads_banner_global);
            ...
            $query_others = 'SELECT a.*,DATE_FORMAT(a.cretime,"%d %M %Y") AS datenya,b.link as link_pages,b.title as cat,b.link as link_pages FROM galeri a LEFT JOIN pages b ON b.id=a.cat WHERE a.hapus=0 AND a.deleted=0 AND a.hidden=0 order by a.cretime  DESC LIMIT  '.$start.','.$limit;
                //echo $query_others;
                $query_others_count = 'SELECT a.*,DATE_FORMAT(a.cretime,"%d %M %Y") AS datenya,b.link as link_pages,(select pages.title FROM pages where record = a.idmenu) as cat,(select pages.link FROM pages where record = a.idmenu) as link_pages FROM galeri a LEFT JOIN pages b ON b.id=a.cat WHERE a.hapus=0 AND a.deleted=0 AND a.hidden=0 order by a.cretime  DESC';
                $this->idpages_sub = $this->db->query('SELECT * FROM pages WHERE parent = '.$this->idpages[0]->id)->result();
                $this->data['subnews']        = $this->idpages_sub;
