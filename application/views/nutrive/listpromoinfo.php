    ...
    <div class="content-row">
        <div class="content-left">
                ...
                <!--<div class="detail_share_socmed">
                    <ul>
                        <li>
                            <a href="javascript:fbShare('<?="http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]"?>', '<?=$detail[0]["title"]?>', '<?=substr($detail[0]["description"], 0, 100)?>', '<?=base_url("uploads/artikel/".$detail[0]["pic2"])?>', 520, 350)"><img src="<?=base_url("assets/frontend/images/share-fb.png")?>"></a>
                ...
                <div class="article">
                    <h3><?=$new?></h3>
                    <div id="promoinfoballon"> 
                      <ul class="promoinfoballon">
                          <li><a id="allpromoinfo" href="<?=base_url("promo-info")?>">All</a></li>
                          <li><a id="justpromo" href="<?=base_url("promo")?>">Promo</a></li>
                          <li><a class="active" id="justinfo" href="<?=base_url("info")?>">Info</a></li>
                      </ul>
                      <div style="position: absolute; color: rgb(255, 255, 255); font-size: 25px; left: -74px; top: 6px;">Filter : </div>
                      </div>
                      <div class="container-box-article">
                        <div class="box_article">
                            <div class="category"><?=$current_page?></div>
                            <div class="owl-carousel">
                                <?php
                                foreach ($new_rec_artikel as $dt){
                                    $descriptionnya = strip_tags($dt["description"]);
                                    if (strlen($descriptionnya) > 150){
                                        $descriptionnya = substr($descriptionnya,0,150)."...";
                                    }
                                ?>
                                <div class="item">
                                    <div class="image-new-article">
                                        <img src="<?=base_url('uploads/artikel/'.$dt["pic"])?>" alt="<?=$dt["title"]?>">
                                    </div>
                                    <div class="text-article-slide">
                                        <h1>
                                            <a title="<?=$dt["title"]?>" href="<?=base_url($codebahasa.$dt["link_detail"].'/'.$dt["link_news"])?>"><?=$dt["title"]?></a>
                                        </h1>
                                        <div class="desc-article-slide"><?=$descriptionnya?></div>
                                        <a title="<?=$dt["title"]?>" href="<?=base_url($codebahasa.$dt["link_detail"].'/'.$dt["link_news"])?>">Selengkapnya &raquo;</a>
                                    </div>
                                </div>
                                <?php
                                }
                    ...
                    ?>
                        ...
                        <li>
                            ...
                            if ($dt["pic"] != ""){
                                ?>
                                <div class="image-article1"><img src="<?=base_url('uploads/artikel/'.$dt["pic"])?>" /></div>
                                <?php
                            }else{
                                ?>
                                <div class="image-article1"><img src="<?=base_url('uploads/artikel/noimage.png')?>" /></div>
                                <?php
                            }
                            ?>
...
$(".loadmore a").click(function () {
    ...
    $.ajax({
        type: "POST",
        url: "<?=base_url("ajax/getpromoinfo")?>",
        //data: { 'a': 1, 'b': 2, 'c': 3 },
        dataType: 'json',
