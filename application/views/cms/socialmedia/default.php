            ...
            <div class="form">
              <div class="col-md-12">
                <div class="form-group">
                  <?php
                  if(isset($status['form']['facebook'][$rowlang['id']]) AND $status['form']['facebook'][$rowlang['id']] != ""){
                    $valuefacebook = $status['form']['facebook'][$rowlang['id']];
                  }else{
                    $valuefacebook = '';
                    if (count($rec) > 0){
                        
                        if (count($rec) > 0){
                            $valuefacebook = $rec[$r]['facebook'];
                        }else{
                            $valuefacebook = '';
                        }
                    }
                  ...
                  <input lang="<?=$rowlang['id']?>" class="input-facebook-key form-control" placeholder="Enter your facebook address" value="<?=$valuefacebook?>" name="facebook[<?=$rowlang['id']?>]" minlength="2" type="text" />
                </div>
                
                <div class="form-group">
                  <?php
                  if(isset($status['form']['twitter'][$rowlang['id']]) AND $status['form']['twitter'][$rowlang['id']] != ""){
                    $valuetwitter = $status['form']['twitter'][$rowlang['id']];
                  }else{
                    $valuetwitter = '';
                    if (count($rec) > 0){
                        
                        if (count($rec) > 0){
                            $valuetwitter = $rec[$r]['twitter'];
                        }else{
                            $valuetwitter = '';
                        }
                    }
                  ...
                  <input lang="<?=$rowlang['id']?>" class="input-twitter-key form-control" placeholder="Enter your twitter address" value="<?=$valuetwitter?>" name="twitter[<?=$rowlang['id']?>]" minlength="2" type="text" />
                </div>
                
                
                <div class="form-group">
                  <?php
                  if(isset($status['form']['googleplus'][$rowlang['id']]) AND $status['form']['googleplus'][$rowlang['id']] != ""){
                    $valuegoogleplus = $status['form']['googleplus'][$rowlang['id']];
                  }else{
                    $valuegoogleplus = '';
                    if (count($rec) > 0){
                        
                        if (count($rec) > 0){
                            $valuegoogleplus = $rec[$r]['googleplus'];
                        }else{
                            $valuegoogleplus = '';
                        }
                    }
                  ...
                  <input lang="<?=$rowlang['id']?>" class="input-googleplus-key form-control" placeholder="Enter your googleplus address" value="<?=$valuegoogleplus?>" name="googleplus[<?=$rowlang['id']?>]" minlength="2" type="text" />
                </div>
