...
<div class="wrapper">
  <div class="row">
    <form role="form" class="cmxform form-horizontal adminex-form" id="commentForm" method="POST" action=""  enctype="multipart/form-data">
          ...
          <div class="panel-body">
            <div class=" form">
              ...
              <div class="col-md-12">
                ...
                <input class="form-control datepicker" style="width: 104px; display: inline-table"  id="tanggal" autocomplete="off" value="<?php echo $rec[0]['crdatenya'];?>" name="tanggal" minlength="2" type="text" required />
                <input class="form-control" style="width: 70px; display: inline-table"  id="time" autocomplete="off" name="time" value="<?php echo $rec[0]['crtime'];?>" minlength="2" type="text" required />
                </div>
                <div class="form-group">
      ...
      <input name="oldpic2<?=$rowlang['id']?>" type="hidden" value="<?=$rec[$r]['pic2']?>">
            ...
            <div class="form">
              <div class="col-md-12">
                <div class="form-group" style="display: none;">
                  ...
                  <?php
                  if(isset($status['form']['show'][$rowlang['id']]) AND $status['form']['show'][$rowlang['id']] != ""){
                    $valuetitle = $status['form']['show'][$rowlang['id']];
                  }else{
                    $valuetitle = $rec[$r]['show'];
                  }
                  ?>
                  ...
                  <select name="show[<?=$rowlang['id']?>]" id="show[<?=$rowlang['id']?>]" class="form-control m-bot15" data-placeholder="Show on <?=$rowlang['title']?> >" id="cdiv">
                    ...
                    <option <?php if($valuetitle == 0){?> selected="selected"  <?php } ?> value="0">No</option>
                    <option <?php if($valuetitle == 1){?> selected="selected"  <?php } ?> value="1">Yes</option>
                  </select>
                </div>
                <div class="form-group">
                  <?php
                  ...
                  <label for="exampleInputEmail1">Image List (*)</label>
                    <div class="fileupload fileupload-new" data-provides="fileupload">
                      <div class="fileupload-new thumbnail" style="width: 500px; height: 200px;">
                        <?php if ($rec[$r]['pic'] != ""): ?>
                        <img src="<?=base_url('uploads/galeri/'.$rec[$r]['pic'])?>" alt=""/>
                        <?php else: ?>
                    ...
                    <span>Witdh : 900px - Height : 655px</span>
                  </div>
                </div>
                ...
                <h2 style="margin-left:-15px;margin-right:-15px;">Ads - Side Banner</h2>
                  <div class="form-group">
                  <?php
                  if(isset($status['form']['link_ads_banner'][$rowlang['id']]) AND $status['form']['link_ads_banner'][$rowlang['id']] != ""){
                    $valuelink_ads_banner = $status['form']['link_ads_banner'][$rowlang['id']];
                  }else{
                    $valuelink_ads_banner = $rec[$r]['link_ads_banner'];
                  }
                  ?>
                  <label for="exampleInputEmail1">Link Ads Banner </label>
                  <input lang="<?=$rowlang['id']?>" class="form-control" placeholder="Enter your title" value="<?=$valuelink_ads_banner?>" name="link_ads_banner[<?=$rowlang['id']?>]"  type="text" />
                </div>
                <input name="old_ads_side_banner<?=$rowlang['id']?>" type="hidden" value="<?=$rec[$r]['ads_side_banner']?>">
                <div class="form-group" style="overflow: hidden;">
                  <div class="col-md-8 lebarwidth" style="padding-left: 0px; overflow: hidden">
                    <div class="fileupload fileupload-new" data-provides="fileupload">
                      <div class="fileupload-new thumbnail" style="width: 500px; height: 200px;">
                        <?php if ($rec[$r]['ads_side_banner'] != ""): ?>
                        <img src="<?=base_url('uploads/ads/'.$rec[$r]['ads_side_banner'])?>" alt=""/>
                        <?php else: ?>
                      ...
                      <div>
                        <span class="btn btn-default btn-file" style="margin-left:0px;margin-right:10px">
                          <span class="fileupload-new"><i class="fa fa-paper-clip"></i> Select image</span>
                          <span class="fileupload-exists"><i class="fa fa-undo"></i> Change</span>
                          <input type="file" class="default" name="ads_side_banner<?=$rowlang['id']?>" />
                        </span>
                        <a href="#" class="btn btn-danger fileupload-exists" data-dismiss="fileupload"><i class="fa fa-trash"></i> Remove</a>
                      </div>
                    </div>
        ...
        $('.input-title-key').blur(function(){
            ...
            if ($(this).val() != ""){
                $.ajax({
                    ...
                    dataType: 'html',
                    async:false,
                    data: { 'lang': $(this).attr("lang"),'title': $(this).val(),'table': 'galeri'},
                    beforeSend: function () {
